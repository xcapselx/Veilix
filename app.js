...
async function initializePolkadotApi() {
  try {
    ...
    // Render the chart
    initializeChart();
    updateChart(lastHeader.number, validatorAddresses.length);
...
function displaySearchResults(posts) {
  ...
  // Function to initialize the Polkadot API and fetch network data
  async function initializePolkadotAPI() {
    try {
      const wsProvider = new WsProvider('wss://rpc.polkadot.io');
      const api = await ApiPromise.create({ provider: wsProvider });
  ...
  async function initializeApplication() {
    try {
      await initializeSubsocial();
      await initializePolkadotAPI();
      initializeWebSocket();
      initializeChart();
      displayUserProfile();
      handleRealTimeNotifications(api);
    } catch (error) {
